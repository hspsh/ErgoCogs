...
module standoffs(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h,tolerance=tolerance, 
    holes_file="mount_holes.dxf")
{
...
module bottom(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h,tolerance=tolerance,outline_file="outer_outline.dxf",holes_file="mount_holes.dxf",sides=true,bottom_space=bottom_space) 
{
  ...
  difference(){
    union(){
      ...
      standoffs(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h,tolerance=tolerance, holes_file=holes_file);
      translate(brain_offset) standoffs(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h,tolerance=1, holes_file="brain_holes.dxf");
    }
    holes(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h,tolerance=tolerance, holes_file=holes_file);
    translate(brain_offset) holes(plate_h=plate_h,bottom_h=bottom_h,side_h=side_h, holes_file="brain_holes.dxf");
    // weight slots
    translate([85, -140, bottom_h-weight_h]) cube([weight_x,weight_y,weight_h]);
    translate([155, -140, bottom_h-weight_h]) cube([weight_x,weight_y,weight_h]);
    
    // IO
    translate([39,-60,bottom_h+bottom_space +0])
        cube([22,22,20]);
...
//all(brain=false,bottom=true,plate=false,left=true,right=true);
